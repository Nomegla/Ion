...
abstract class FluidType(override val key: IonRegistryKey<FluidType, out FluidType>) : Keyed<FluidType> {
	...
	open fun getHeatingResult(
		...
		val newTemperature = FluidUtils.getNewTemperature(
			stack,
			appliedEnergyJoules * HEATING_RATE_MULTIPLIER,
			maximumTemperature,
			location
		)
	...
	sealed class HeatingResult(val newTemperature: FluidProperty.Temperature) {
		...
		companion object {
			/** Global gameplay multiplier because physically realistic heating rates are too slow. */
