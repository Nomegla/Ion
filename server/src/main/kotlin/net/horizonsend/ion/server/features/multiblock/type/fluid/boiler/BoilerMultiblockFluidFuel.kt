...
object BoilerMultiblockFluidFuel : BoilerMultiblock<FluidBoilerEntity>() {
	...
	override fun MultiblockShape.buildStructure() {
		z(6) {
			y(-1) {
				x(-3).anyWall()
				x(-2).ironBlock()
				x(-1).anyStairs(PrepackagedPreset.stairs(
					RelativeFace.BACKWARD, Bisected.Half.BOTTOM, shape = Stairs.Shape.STRAIGHT
				))
			...
			y(1) {
				x(-2).anyStairs(PrepackagedPreset.stairs(
					RelativeFace.BACKWARD, Bisected.Half.BOTTOM, shape = Stairs.Shape.STRAIGHT
				))
				x(-1).anyTerracotta()
				x(0).fluidPort()
		...
		z(0) {
			...
			y(0) {
				...
				x(2).anyCustomBlockOrMaterial(
					...
					listOf(Material.IRON_BLOCK),
					"redstone control port or iron block"
				) { setExample(Material.IRON_BLOCK) }
				x(3).anyWall()
			}
			y(1) {
	...
	class FluidBoilerEntity(
		...
		val fuelStorage = FluidStorageContainer(
			data, "fuel_storage", text("Fuel Storage"), NamespacedKeys.key("fuel_storage"), 100_000.0,
			FluidRestriction.FluidPropertyWhitelist(FLAMMABILITY)
		)
		val pollutionStorage = FluidStorageContainer(
			data, "pollution_out", text("Pollution Output"), NamespacedKeys.key("pollution_out"), 100_000.0,
			FluidRestriction.Unlimited
		)

		override fun IOData.Builder.registerAdditionalIO(): IOData.Builder = addPort(IOType.FLUID, 0, -1, 0) {
			IOPort.RegisteredMetaDataInput(this@FluidBoilerEntity, FluidPortMetadata(
				connectedStore = fuelStorage,
				inputAllowed = true,
				outputAllowed = false
			))
		}.addPort(IOType.FLUID, 0, 1, 6) {
			IOPort.RegisteredMetaDataInput(this@FluidBoilerEntity, FluidPortMetadata(
				connectedStore = pollutionStorage,
				inputAllowed = false,
				outputAllowed = true
			))
		}
		...
		override val displayHandler: TextDisplayHandler = DisplayHandlers.newMultiblockSignOverlay(
			this,
			{
				ComplexFluidDisplayModule(
					handler = it, container = fluidInput, title = text("Input"), offsetLeft = 3.5,
					offsetUp = 1.15, offsetBack = -4.0 + 0.39, scale = 0.7f, RelativeFace.RIGHT
				)
			},
			{
				ComplexFluidDisplayModule(
					handler = it, container = fluidOutput, title = text("Output"), offsetLeft = -3.5,
					offsetUp = 1.15, offsetBack = -4.0 + 0.39, scale = 0.7f, RelativeFace.LEFT
				)
			},
			{
				SimpleFluidDisplayModule(
					handler = it, storage = fuelStorage, offsetLeft = 0.0, offsetUp = getLinePos(2),
					offsetBack = 0.0, scale = MATCH_SIGN_FONT_SIZE
				)
			},
			{
				StatusDisplayModule(
					handler = it, statusSupplier = statusManager, offsetLeft = 0.0, offsetUp = getLinePos(4),
					offsetBack = 0.0, scale = MATCH_SIGN_FONT_SIZE
				)
			}
		)
		...
		private var fuelBurnedThisUpdate = 0.0
		private var heatProducedThisUpdate = 0.0

		override fun preTick(): Boolean {
			val combustionContents = fuelStorage.getContents()
			if (combustionContents.isEmpty()) {
				clearPendingBurn()
				return false
			}

			val combustionResult = combustionContents.getData(FLAMMABILITY)
			if (combustionResult == null) {
				clearPendingBurn()
				return false
			}

			fuelBurnedThisUpdate = minOf(
				BURNED_PER_TICK * tickingManager.interval,
				combustionContents.amount
			)
			heatProducedThisUpdate = combustionResult.joulesPerLiter * fuelBurnedThisUpdate

			// Pollution production and its capacity check are intentionally disabled for now.
			// val pollutionStack =
			// 	FluidStack(
			// 		combustionResult.resultFluid,
			// 		fuelBurnedThisUpdate * combustionResult.resultVolumeMultiplier
			// 	)
			// return pollutionStorage.canAdd(pollutionStack)

			return fuelBurnedThisUpdate > 0.0
		}
		...
		private fun consumeFuel() {
			...
			fuelStorage.removeAmount(fuelBurnedThisUpdate)
			// pollutionStorage.addFluid(pollutionStack, location)
			clearPendingBurn()
		}

		private fun clearPendingBurn() {
			fuelBurnedThisUpdate = 0.0
		...
		companion object {
			private const val BURNED_PER_TICK = 0.5
		}
	}
}
