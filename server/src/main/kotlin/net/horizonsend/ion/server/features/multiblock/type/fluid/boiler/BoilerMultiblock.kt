...
abstract class BoilerMultiblock<T : BoilerMultiblockEntity> : Multiblock(), EntityMultiblock<T> {
	...
	abstract class BoilerMultiblockEntity(
	...
	) : MultiblockEntity(manager, multiblock, world, x, y, z, structureDirection), DisplayMultiblockEntity,
		AsyncTickingMultiblockEntity, FluidStoringMultiblock, StatusMultiblockEntity, RedstoneControlledMultiblock {
		...
		val fluidInput = FluidStorageContainer(
			data, "primaryin", text("Primary Input"), NamespacedKeys.key("primaryin"), 10_000.0,
			FluidRestriction.Unlimited
		)
		val fluidOutput = FluidStorageContainer(
			data, "primaryout", text("Primary Output"), NamespacedKeys.key("primaryout"), 100.0,
			FluidRestriction.Unlimited
		)
		...
		override fun tickAsync() {
			 ...
			 *     val temperature =
			 *         outputContents.getDataOrDefault(FluidPropertyTypeKeys.TEMPERATURE, location).value
			 *     if (temperature > 600.0 && fluidOutput.getRemainingRoom() <= 0) {
			 *         Tasks.sync {
			 *             val explosionLocation = getBlockRelative(0, 5, 3).location.toCenterLocation()
			 *             world.createExplosion(explosionLocation, 30.0f)
			 *         }
			 *         fluidOutput.clear()
			 *         fluidInput.clear()
			 *         return
		...
		fun heatFluid(deltaSeconds: Double) {
			...
			when (heatingResult) {
				...
				is FluidType.HeatingResult.TemperatureIncreasePassthrough -> {
					transferToOutput(input)
					setStatus(FluidPropertyTypeKeys.TEMPERATURE.getValue().formatValue(heatingResult.newTemperature))
				}
				is FluidType.HeatingResult.Boiling -> {
					addBoilingResult(heatingResult)
					setStatus(ofChildren(
						text("Boiling "),
						FluidPropertyTypeKeys.TEMPERATURE.getValue().formatValue(heatingResult.newTemperature)
					))
				}
			}
		}

		private fun transferToOutput(input: FluidStack) {
			if (!fluidOutput.canAdd(input)) return

			val transferAmount = minOf(input.amount, fluidOutput.getRemainingRoom())
			if (transferAmount <= 0.0) return

			fluidOutput.addFluid(input.asAmount(transferAmount), location)
			fluidInput.removeAmount(transferAmount)
		}

		private fun addBoilingResult(result: FluidType.HeatingResult.Boiling) {
			val output = result.newFluidStack
			if (output.isEmpty() || !fluidOutput.canAdd(output)) return

			val acceptedOutput = minOf(output.amount, fluidOutput.getRemainingRoom())
			if (acceptedOutput <= 0.0) return

			val acceptedFraction = acceptedOutput / output.amount
			fluidOutput.addFluid(output.asAmount(acceptedOutput), location)
